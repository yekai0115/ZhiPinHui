...
public class CitycodeUtil {
	...
	public ArrayList<String> getProvince(List<Cityinfo> provice) {
		...
		for (int i = 0; i < provice.size(); i++) {
			province_list.add(provice.get(i).getRegion_name());
			province_list_code.add(provice.get(i).getRegion_id());
		}
...
//		System.out.println("city--->" + city.toString());
		for (int i = 0; i < city.size(); i++) {
			city_list.add(city.get(i).getRegion_name());
			city_list_code.add(city.get(i).getRegion_id());
		}
		return city_list;
...
//		System.out.println("couny--->" + couny.toString());
		for (int i = 0; i < couny.size(); i++) {
			couny_list.add(couny.get(i).getRegion_name());
			couny_list_code.add(couny.get(i).getRegion_id());
		}
		return couny_list;
