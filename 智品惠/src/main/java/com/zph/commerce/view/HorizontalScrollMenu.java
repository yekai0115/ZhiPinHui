...
import com.zph.commerce.widget.viewpager.MyViewPager;
...
{
	private HorizonScrolAdapter mAdapter;
	...
	public void setAdapter(HorizonScrolAdapter adapter)
	{
		if (null != adapter)
		{
	 ...
	 * @param adapter
	 */
	private void initView(HorizonScrolAdapter adapter)
	{
		if (null == adapter)
		{
			return;
	 ...
	 * @param adapter
	 */
	public void notifyDataSetChanged(HorizonScrolAdapter adapter)
	{
	...
	public  void scrollTo(String categoty){

		for (int i = 0; i < mItems.size(); i++) {

			String str=mItems.get(i);
			if(categoty.equals(str)){
				RadioButton btn=rb_items.get(i);
				setMenuItemsNullBackground();
                btn.setBackground(getResources().getDrawable(R.drawable.bg_cirle_cad_tyle));
